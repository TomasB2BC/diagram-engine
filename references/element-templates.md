...
**Key properties:**
- `roughness: 1` -- "Architect" sloppiness (slightly organic)
- `endArrowhead: "triangle_outline"` -- open triangle tip (cleaner than default chevron)
- `roundness: {"type": 2}` -- rounded bends at elbow points
- `elbowed: true` -- auto-routes with right-angle paths

For elbow routing: use 2+ points in `points` array. Excalidraw auto-routes the path between them with right-angle bends.
